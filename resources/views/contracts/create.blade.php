...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="h3 mb-1"><i class="fas fa-plus-circle me-3 text-primary"></i>Create New Contract</h1>
        <p class="text-muted mb-0">Fill in the details to add a new client contract.</p>
    </div>
    ...
    <div class="card-body">
        <form action="{{ route('contracts.store') }}" method="POST">
                    ...
                    @foreach(old('services') as $index => $oldService)
                        <div class="row service-item mb-3 p-3 border rounded bg-light position-relative">
                            <div class="col-md-5 mb-3">
                                ...
                                <select class="form-select service-type-select @error('services.' . $index . '.service_type_id') is-invalid @enderror" id="service_type_id_{{ $index }}" name="services[{{ $index }}][service_type_id]" required>
                                    ...
                                    @foreach($serviceTypes as $serviceType)
                                        <option value="{{ $serviceType->id }}" data-installation-tzs="{{ $serviceType->installation_price_tzs }}" data-monthly-tzs="{{ $serviceType->monthly_price_tzs }}" data-installation-usd="{{ $serviceType->installation_price_usd }}" data-monthly-usd="{{ $serviceType->monthly_price_usd }}" {{ $oldService['service_type_id'] == $serviceType->id ? 'selected' : '' }}>
                                            {{ $serviceType->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-3 mb-3">
                                ...
                                <input type="number" class="form-control service-quantity @error('services.' . $index . '.quantity') is-invalid @enderror" id="quantity_{{ $index }}" name="services[{{ $index }}][quantity]" value="{{ $oldService['quantity'] }}" min="1" required>
            ...
            <div class="mt-4 text-end">
                <button type="submit" class="btn btn-success"><i class="fas fa-save me-2"></i>Save Contract</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Initial setup for existing services (if any, from old('services'))
        ...
        // Ensure costs are calculated on page load for old inputs
        updateCosts();
    });
</script>
@endpush
